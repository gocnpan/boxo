...
### Testing
Boxo maintainers are in the process of moving tests into Boxo from Kubo (see [#275](https://github.com/gocnpan/boxo/issues/275)) . Until that's done, Boxo releases rely on Kubo tests for extra confidence. As a result releases must plumb Boxo into Kubo and ensure its tests pass. See [Release Process](#release-process) for details.
...
We intend to use the standardized changelog to automate releases by onboarding with [Changelog Driven Release](https://github.com/pl-strflt/changelog-driven-release) (see [#269](https://github.com/gocnpan/boxo/issues/269)).

At least as of 2023-06-08, changelog test is manually copied from [the changelog](CHANGELOG.md) to https://github.com/gocnpan/boxo/releases.  This means that any updates to the changelog after a release need to be manually copied as well.

### Related Work
Below are links of related/adjacent work that has informed some of the decisions in this document:
1. https://github.com/gocnpan/boxo/issues/170
...
## Release Process
1. Create an issue for the version: https://github.com/gocnpan/boxo/issues/new?title=Release+X.Y.Z (can be blank for now)
2. Pin the issue
...
- [ ] Ensure Kubo tests are passing
  - [ ] Go to Kubo dir and run `go get github.com/gocnpan/boxo@<commit_hash>` using the commit hash of the `release-vX.Y.Z` branch
